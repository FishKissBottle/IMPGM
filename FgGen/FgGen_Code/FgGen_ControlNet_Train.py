...
def _maybe_preload_controlnet_weights(model):
    """Optionally initialize the FgGen ControlNet branch from EMA weights."""
    if not PRELOAD_ENABLED:
        return
    if not PRELOAD_SOURCE_PATH:
        raise ValueError("FgGen ControlNet preloading is enabled but source_path is empty.")
    if not Path(PRELOAD_SOURCE_PATH).is_file():
        raise FileNotFoundError(f"FgGen ControlNet preload source not found: {PRELOAD_SOURCE_PATH}")
...
@torch.no_grad()
def _draw_fggen_controlnet_microbatched(
    draw_labs,
    draw_msks,
    controlnet_model,
    vae_model,
    ...
    try:
        ...
        for mb_start, mb_end in iter_microbatch_slices(batch_size, microbatch_size):
            draw_outputs.append(
                FgGen_ControlNet_Denoise(
                    sampler_mode=DRAW_SAMPLER_MODE,
                    prompt_str=slice_microbatch(draw_labs, mb_start, mb_end),
                    conditional_element=slice_microbatch(draw_msks, mb_start, mb_end),
...
class ControlNetLossCalculator(nn.Module):
    """Compute the training loss for the FgGen ControlNet."""
    ...
    def forward(self, x_0, prompt_str, conditional_element):
        ...
        pred_v_t = self.model(x_t, t, prompt_str, conditional_element)
...
def train(
    ...
    with tqdm(data_loader, dynamic_ncols=False, colour="#ff924a", leave=False, ncols=120) as data:
        for labs, fg_imgs, msks, _, _, _ in data:
            if benchmark_recorder is not None:
                benchmark_recorder.before_step((labs, fg_imgs, msks))
            ...
            for mb_start, mb_end in iter_microbatch_slices(batch_size, microbatch_size):
                ...
                with build_train_autocast():
                    ...
                    loss, sp_loss, edge_loss = loss_calculator(fg_imgs_e, labs_mb, msks_mb)
...
def benchmark_training_memory(
    *, output, warmup_steps=5, measure_steps=20, batch_size=1,
    ...
    """Measure the formal FgGen ControlNet training path without saving."""
    ...
    control_model = ControlNet_on_FgGen_Diffusion(
    ...
    _maybe_preload_controlnet_weights(control_model)
    trainable_params = [
        parameter for parameter in control_model.parameters()
        if parameter.requires_grad
    ]
    optimizer = torch.optim.Adam(
        trainable_params, lr=LEARNING_RATE, betas=ADAM_BETAS
    )
    ...
    ema_control_model = build_ema_model(control_model.ControlNet_model)
    ...
    recorder = TrainingMemoryRecorder(
        method_key="impgm",
        stage="fggen_controlnet_training",
        output=output,
        ...
        notes="Frozen FgGen diffusion and shared VAE remain loaded as in formal training.",
    )
    recorder.bind_trainable_modules([control_model])
    train(
        train_loader,
        latent_model,
        optimizer,
...
def valid(data_loader, Latent_model, loss_calculator, latent_scaling_factor):
    ...
    with torch.no_grad():
        for labs, fg_imgs, msks, _, _, _ in data_loader:
            batch_size = fg_imgs.shape[0]
...
def main(resume_train=False):
    if TASK_NAME != "fggen_controlnet":
        raise RuntimeError(
            f"FgGen_ControlNet_Train requires fggen_controlnet.yaml, got {TASK_NAME!r}"
        )
    ...
    if os.path.exists(MODEL_SAVEPATH) and not resume_train:
        raise Exception('FGGEN_CONTROLNET_MODEL exists and resume_train is False ^_^')

    FgGen_ControlNet_model = ControlNet_on_FgGen_Diffusion(
        FgGen_Diffusion_model_savepath=FGGEN_BASE_DIFFUSION_MODEL_SAVEPATH,
        ch=MODEL_CH,
        out_ch=MODEL_OUT_CH,
        ch_mult=MODEL_CH_MULT,
    ...
    if not resume_train:
        _maybe_preload_controlnet_weights(FgGen_ControlNet_model)

    trainable_params = [
        param for param in FgGen_ControlNet_model.parameters() if param.requires_grad
    ]
    optimizer = torch.optim.Adam(
        trainable_params,
        lr=LEARNING_RATE,
    ...
    loss_calculator = ControlNetLossCalculator(
        FgGen_ControlNet_model, beta_t, vae_model=Latent_model, latent_scaling_factor=latent_scaling_factor
    ).to(DEVICE)
    lr_scheduler = build_scheduler(optimizer, MIN_LEARNING_RATE, PATIENCE_THRESHOLD_NUM)
    ema_control_model = build_ema_model(FgGen_ControlNet_model.ControlNet_model)
    resume_extra = {}

    if resume_train:
        FgGen_ControlNet_model, optimizer, lr_scheduler, last_epoch, last_loss_dict, resume_extra, _ = load_model(
            MODEL_SAVEPATH,
    ...
    draw_labs, draw_fg_imgs, draw_msks, _, draw_projs, draw_geos = next(iter(draw_loader))
    ...
    for epoch_idx in range(start_epoch, TRAIN_MAX_EPOCHS):
        ...
        if resume_train and epoch_idx == start_epoch:
            ...
            if DETERMINISTIC_EVAL:
                set_random_seed(RANDOM_SEED, deterministic=False)
            with use_controlnet_branch(FgGen_ControlNet_model, ema_control_model):
                FgGen_ControlNet_model.eval()
        ...
        writer.add_scalars('FgGen_ControlNet/train_loss',      {f"{_tag_prefix}_train_loss": train_loss},      epoch_num)
        writer.add_scalars('FgGen_ControlNet/train_sp_loss',   {f"{_tag_prefix}_train_sp_loss": train_sp_loss}, epoch_num)
        writer.add_scalars('FgGen_ControlNet/train_edge_loss', {f"{_tag_prefix}_train_edge_loss": train_edge_loss}, epoch_num)
        writer.add_scalars('FgGen_ControlNet/valid_loss',      {f"{_tag_prefix}_valid_loss": valid_loss},      epoch_num)
        writer.add_scalars('FgGen_ControlNet/valid_sp_loss',   {f"{_tag_prefix}_valid_sp_loss": valid_sp_loss}, epoch_num)
        writer.add_scalars('FgGen_ControlNet/valid_edge_loss', {f"{_tag_prefix}_valid_edge_loss": valid_edge_loss}, epoch_num)
        ...
        if epoch_num == 1:
            rgb_save_path = os.path.join(RGB_DIR, 'FgGen_ControlNet_RGB_1.png')
            tif_save_path = os.path.join(TIF_DIR, 'FgGen_ControlNet_TIF_1.tif')
            msk_save_path = os.path.join(RGB_DIR, 'FgGen_ControlNet_MSK_1.png')
            save_rgb_datas(
                prepare_rgb_vis_tensor(draw_fg_imgs, masks=draw_msks),
                nrow=3,
            ...
            save_msk_datas(draw_msks, nrow=3, savepath=msk_save_path)
        ...
        if epoch_num % DRAW_INTERVAL_EPOCHS == 0:
            print(f'   FgGen_ControlNet at epoch: {epoch_num} --- labels: {draw_labs}')
            with use_controlnet_branch(FgGen_ControlNet_model, ema_control_model):
                FgGen_ControlNet_model.eval()
                Denoised_Fg_Imgs = _draw_fggen_controlnet_microbatched(
                    draw_labs,
                    draw_msks,
                    controlnet_model=FgGen_ControlNet_model,
                    vae_model=Latent_model,
                    draw_projs=draw_projs,
                    draw_geos=draw_geos,
                )
            save_rgb_datas(
                prepare_rgb_vis_tensor(Denoised_Fg_Imgs),
                nrow=3,
                savepath=os.path.join(RGB_DIR, f'FgGen_ControlNet_RGB_{epoch_num}.png'),
                is_showminmax=True,
            )
            if SAVE_TIF_IMAGES and epoch_num % TIF_INTERVAL_EPOCHS == 0:
                save_tif_datas(
                    ...
                    projections=draw_projs,
                    geotransforms=draw_geos,
                    savepath=os.path.join(TIF_DIR, f'FgGen_ControlNet_TIF_{epoch_num}.tif'),
                    is_showminmax=True,
                )
