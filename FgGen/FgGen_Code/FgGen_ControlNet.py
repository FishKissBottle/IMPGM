...
class ControlNet(nn.Module):
    """Encode foreground conditions into ControlNet residual features."""
    def __init__(self, 
                 ch=MODEL_CH,
...
class ControlNet_on_FgGen_Diffusion(nn.Module):
    """Combine the FgGen diffusion backbone with its ControlNet branch."""
    def __init__(self,
        ...
        FgGen_Diffusion_model_exist = os.path.exists(FgGen_Diffusion_model_savepath)
        if not FgGen_Diffusion_model_exist:
            raise FileNotFoundError(
                f"FgGen diffusion checkpoint does not exist: {FgGen_Diffusion_model_savepath}"
            )
        FgGen_Diffusion_model = FgGen_Diffusion_UNet(ch=ch,
                                                     out_ch=out_ch,
                                                     ch_mult=ch_mult,
    ...
    def train(self, mode=True):
        ...
        super().train(mode)
        self.FgGen_Diffusion_model.eval()
        return self
    ...
    def forward(self, x, timesteps, prompt_str, conditional_element):
        ...
        prompt_idx = torch.tensor(
            [self.FgGen_Diffusion_model.prompt_dict[i] for i in prompt_str],
            device=x.device,
        )
        prompt_embed_res_attn = self.FgGen_Diffusion_model.prompt_embd_attn(prompt_idx)
        
        temb = get_timestep_embedding(timesteps, self.FgGen_Diffusion_model.ch)
        temb = self.FgGen_Diffusion_model.temb.dense[0](temb)
        temb = nonlinearity(temb)
        temb = self.FgGen_Diffusion_model.temb.dense[1](temb)

        # downsampling
        hs = [self.FgGen_Diffusion_model.conv_in(x)]
        for i_level in range(self.FgGen_Diffusion_model.num_resolutions):
            downblocks_num = len(self.FgGen_Diffusion_model.down[i_level].block_a)
            for i_block in range(downblocks_num):

                h = self.FgGen_Diffusion_model.down[i_level].block_a[i_block](hs[-1], temb)

                if i_block == downblocks_num - 1:
                    for idx in range(len(self.FgGen_Diffusion_model.down[i_level].self_attn)):
                        if len(self.FgGen_Diffusion_model.down[i_level].self_attn) > 0:
                            h = self.FgGen_Diffusion_model.down[i_level].self_attn[idx](h)
                        if len(self.FgGen_Diffusion_model.down[i_level].cross_attn) > 0:
                            h = self.FgGen_Diffusion_model.down[i_level].cross_attn[idx](h, prompt_embed_res_attn)
                        if len(self.FgGen_Diffusion_model.down[i_level].block_b) > 0:
                            h = self.FgGen_Diffusion_model.down[i_level].block_b[idx](h, temb)

                hs.append(h)

            if i_level != self.FgGen_Diffusion_model.num_resolutions - 1:
                hs.append(self.FgGen_Diffusion_model.down[i_level].downsample(hs[-1]))

        # middle
        h = self.FgGen_Diffusion_model.mid.block_1(hs[-1], temb)
        h = self.FgGen_Diffusion_model.mid.self_attn_1(h)
        h = self.FgGen_Diffusion_model.mid.cross_attn_1(h, prompt_embed_res_attn)
        h = self.FgGen_Diffusion_model.mid.block_2(h, temb)
        h = self.FgGen_Diffusion_model.mid.self_attn_2(h)
        h = self.FgGen_Diffusion_model.mid.cross_attn_2(h, prompt_embed_res_attn)
        h = self.FgGen_Diffusion_model.mid.block_3(h, temb)
        ...
        # upsampling
        for i_level in reversed(range(self.FgGen_Diffusion_model.num_resolutions)):
            upblocks_num = len(self.FgGen_Diffusion_model.up[i_level].block_a)
            for i_block in range(upblocks_num):

                h = self.FgGen_Diffusion_model.up[i_level].block_a[i_block](torch.cat([h, hs.pop() + self.ControlNet_weight * control_connections.pop()], dim=1), temb)

                if i_block == upblocks_num - 1:
                    for idx in range(len(self.FgGen_Diffusion_model.up[i_level].self_attn)):
                        if len(self.FgGen_Diffusion_model.up[i_level].self_attn) > 0:
                            h = self.FgGen_Diffusion_model.up[i_level].self_attn[idx](h)
                        if len(self.FgGen_Diffusion_model.up[i_level].cross_attn) > 0:
                            h = self.FgGen_Diffusion_model.up[i_level].cross_attn[idx](h, prompt_embed_res_attn)
                        if len(self.FgGen_Diffusion_model.up[i_level].block_b) > 0:
                            h = self.FgGen_Diffusion_model.up[i_level].block_b[idx](h, temb)

            if i_level != 0:
                h = self.FgGen_Diffusion_model.up[i_level].upsample(h)

        # end
        h = self.FgGen_Diffusion_model.norm_out(h)
        h = nonlinearity(h)
        h = self.FgGen_Diffusion_model.conv_out(h)
