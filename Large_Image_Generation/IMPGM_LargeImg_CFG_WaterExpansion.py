...
class Generate_LargeImg_CFG(nn.Module):
    """Generate large tiled images for the water-expansion CFG scenario."""
    def __init__(self, 
                 sampler_mode, 
                 beta_t,
                 overlap_rate,
        ...
        if dem_data is None:
            raise ValueError("Water expansion requires dem_data; DEM input must not be None.")
    ...
    def build_expanded_water_mask(
        ...
        modified_msk, delta_map = rule.grow_inundation(
            delta_num=delta_num,
    ...
    def _get_cloud_prompt(self, cur_prompt_str, cloud_prompt_str_list, nocloud_prompt_str, LargeImg_e, row_idx, col_idx, detect_pixel_thx, 
        ...
        if row_idx == 0 and col_idx == 0:
            ...
            if row_idx == 0 and col_idx > 0:
                left_overlap_area = MskSlice_left[:, :, :, self.ImgSlice_w - detect_pixel_thx:]
                if torch.sum(left_overlap_area) >= 1:
                    # Cloud detected in the overlap area, sample from cloud_prompt_str_list
    ...
    def _compute_grad_of_crossarea(self, 
        ...
        Vertical_GradStrip_0to1 = GradStripSeq_0to1.expand(ImgSlice_h, overlap_thx).unsqueeze(0).unsqueeze(0).clone()    # vertical strip
        Vertical_GradStrip_1to0 = 1.0 - Vertical_GradStrip_0to1                                                          # inverted vertical strip
    ...
    def main(self, LargeImg_h, LargeImg_w, first_obj_prompt_str, obj_prompt_str, noobj_prompt_str, first_cloud_prompt_str, 
        ...
        for idx, diagonal_points in enumerate(diagonals_points_list):
            ...
            # Generate and write into ObjFgLargeImg_e, CloudFgLargeImg_e and SynLargeImg_e
            if row_idx == 0 and col_idx == 0: 
                print('ObjPrompt_list: ', ObjPrompt_list)
                print('CloudPrompt_list: ', CloudPrompt_list)

                if not self.is_with_ControlNet:
                    # Dilating cloud masks softens their boundaries.
                    ...
                    # Inpaint the Obj part in SynSlice_condele_set first
                ...
                for Loop_idx in range(Loop_num):
                    ...
                    if not self.is_with_ControlNet:
                        # Keep the blending mask unchanged to avoid visible seams.
                        ...
                        # Inpaint the Obj part in SynSlice first
        ...
        if self.ImgSyn_TIF_savepath is not None:
            _, saved_proj, saved_geotrans = save_tif_datas(SynLargeImg.cpu(), projections=saved_proj, geotransforms=saved_geotrans, savepath=self.ImgSyn_TIF_savepath, is_showminmax=False)  
...
if __name__=='__main__':
    ...
    h_start_idx, h_end_idx =  192,  448
    w_start_idx, w_end_idx = 1092, 1348
    ...
    ModifiedMsk_data, delta_map, score_map = Generate_LargeImg_CFG.build_expanded_water_mask(
        obj_data=Img_data,
        msk_data=Msk_data,
        dem_data=DEM_data,
        slope_data=Slope_data,
    ...
    Generate_LargeImg_CFG(sampler_mode='ddpm',
                          beta_t=beta_t,
                          overlap_rate=0.25,
                          ...
                          conditional_element_geotrans=[Img_geotransform],
                          ).main(LargeImg_h=256, LargeImg_w=256, 
                                 ...
                                 FgGen_original_msk=ModifiedMsk_data,
