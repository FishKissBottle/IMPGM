...
from RuleSet.RuleSet import Water_RiseFall_Rule

class Generate_LargeImg_CFG(nn.Module):
    """Generate large tiled images for the water-contraction CFG scenario."""
    def __init__(self, 
                 sampler_mode, 
                 beta_t,
                 overlap_rate,
        ...
        if dem_data is None:
            raise ValueError("Water contraction requires dem_data; DEM input must not be None.")
        dem_data = np.asarray(dem_data)
        if dem_data.ndim != 2:
            raise ValueError(f"dem_data must be a two-dimensional array, got shape {dem_data.shape}.")
    ...
    @staticmethod
    def build_contracted_water_mask(
        obj_data,
        msk_data,
        dem_data,
        slope_data,
        ...
        modified_msk, delta_map = rule.shrink_inundation(
            delta_num=delta_num,
            delta_rate=delta_rate,
            score=score_map,
        )
        return modified_msk, delta_map, score_map
    ...
    def _get_cloud_prompt(self, cur_prompt_str, cloud_prompt_str_list, nocloud_prompt_str, LargeImg_e, row_idx, col_idx, detect_pixel_thx, 
        ...
        # Determine the prompt order by probability
        ...
        if row_idx == 0 and col_idx == 0:
            ...
            if row_idx == 0 and col_idx > 0:
                left_overlap_area = MskSlice_left[:, :, :, self.ImgSlice_w - detect_pixel_thx:]
                if torch.sum(left_overlap_area) >= 1:
                    # Cloud detected in the overlap region; sample from cloud_prompt_str_list
    ...
    def _compute_grad_of_crossarea(self, 
        ...
        GradStripSeq_0to1 = torch.cat((torch.zeros(zeros_thx, dtype=torch.float32),
                                       ...
                                       )).to(DEVICE)                                                                     # 1-D gradient strip
        Vertical_GradStrip_0to1 = GradStripSeq_0to1.expand(ImgSlice_h, overlap_thx).unsqueeze(0).unsqueeze(0).clone()    # vertical gradient strip, 0 -> 1
        Vertical_GradStrip_1to0 = 1.0 - Vertical_GradStrip_0to1                                                          # vertical gradient strip, 1 -> 0
    ...
    def main(self, LargeImg_h, LargeImg_w, first_obj_prompt_str, obj_prompt_str, noobj_prompt_str, first_cloud_prompt_str, 
        ...
        if self.dem_data.shape != requested_size:
            raise ValueError(
                f"dem_data must have spatial size {requested_size}, got {self.dem_data.shape}."
            )
        ...
        for idx, diagonal_points in enumerate(diagonals_points_list):
            ...
            # Start generation and write into ObjFgLargeImg_e, CloudFgLargeImg_e and SynLargeImg_e
            if row_idx == 0 and col_idx == 0: 
                ...
                if not self.is_with_ControlNet:
                    ...
                    # First inpaint the Obj regions in SynSlice_condele_set
                    FgSlice_orimsk_e_set = F.interpolate(ObjFgSlice_orimsk_list[0], size=(IMG_SIZE // SIDELENGTH_SCALE_FACTOR, IMG_SIZE // SIDELENGTH_SCALE_FACTOR), mode='nearest')
                    FgSlice_orimsk_e_set = tensor_dilate(FgSlice_orimsk_e_set)
                    FgSlice_orimsk_e_set = gaussian_blur(FgSlice_orimsk_e_set, kernel_size=5, sigma=3)
                    ...
                    # Then extract the high-frequency signal
                ...
                for Loop_idx in range(Loop_num):
                    ...
                    if not self.is_with_ControlNet:
                        ...
                        # First inpaint the Obj regions in SynSlice
                        ...
                        # Then run the denoising synthesis   
                        prompt_str_dict = {
                            'prompt_str_NoObj': ['NoObj' for _ in range(len(ObjPrompt_batch))],
                            'prompt_str_Obj': ObjPrompt_batch,
                            'prompt_str_NoCloud': ['NoCloud' for _ in range(len(CloudPrompt_batch))],
            ...
            # Write back into the large latent images
            for idx, (ObjFgSlice_e, CloudFgSlice_e, SynSlice_e) in enumerate(zip(ObjFgSlice_e_set, CloudFgSlice_e_set, SynSlice_e_set)):
                ...
                # Left edge
                    ...
                    # Adjust H_MosGradMap_1to0 and H_MosGradMap_0to1
                    ...
                    if row_idx != row_idx_max:
                        ...
                        # Note: at row_idx_max there is no need to subtract self.overlap_buffer_thx_e
        ...
        # Decode and write into the large images
...
if __name__=='__main__':
    ...
    ModifiedMsk_data, delta_map, score_map = Generate_LargeImg_CFG.build_contracted_water_mask(
        obj_data=Img_data,
        msk_data=Msk_data,
        dem_data=DEM_data,
        slope_data=Slope_data,
    ...
    # Convert Msk_data, ModifiedMsk_data and Img_data to tensors and preprocess them
    Msk_data = torch.from_numpy(Msk_data).to(device=DEVICE, dtype=torch.float32)
    Msk_data = Msk_data.unsqueeze(0).unsqueeze(0)

    ModifiedMsk_data = torch.from_numpy(ModifiedMsk_data).to(device=DEVICE, dtype=torch.float32)
    ModifiedMsk_data = ModifiedMsk_data.unsqueeze(0).unsqueeze(0)
    ...
    Generate_LargeImg_CFG(sampler_mode='ddim',
                          ...
                          is_FgGenInpaint_Resample=True,
                          is_ImgSynInpaint_Resample=False,
                          is_Fixed_CloudPrompt=True,
                          ...
                          conditional_element_geotrans=[Img_geotransform],
                                 ...
                                 first_cloud_prompt_str='NoCloud', cloud_prompt_str_list=['FewCloud', 'LessCloud', 'MoreCloud', 'ManyCloud'], nocloud_prompt_str='NoCloud',
                                 FgGen_original_msk=Msk_data,
                                 FgGen_conditional_element=ModifiedMsk_data, ImgSyn_conditional_element=Img_data)
