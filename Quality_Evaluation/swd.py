...
def get_gaussian_kernel(device="cpu", dtype=torch.float32):
    kernel = np.array([
        [1, 4, 6, 4, 1],
        [4, 16, 24, 16, 4],
        [6, 24, 36, 24, 6],
    ...
    gaussian_k = torch.as_tensor(
        kernel.reshape(1, 1, 5, 5),
        device=device,
        dtype=dtype,
    )
    return gaussian_k
...
def pyramid_down(image, device="cpu"):
    gaussian_k = get_gaussian_kernel(device=device, dtype=image.dtype)
...
def pyramid_up(image, device="cpu"):
    ...
    multiband = [
        F.conv2d(upsample[:, i:i + 1, :, :], gaussian_k, padding=2)
        for i in range(image.shape[1])
    ]
...
def extract_patches(pyramid_layer, slice_indices,
    ...
    x = (x - mean) / (std + 1e-8)
    # reshape to 2rank
    x = x.reshape(-1, pyramid_layer.size(1) * slice_size * slice_size)
    return x
...
def swd(image1, image2,
        ...
        pyramid_batchsize=128, generator=None):
    ...
    if image1.ndim != 4 or image2.ndim != 4:
        raise ValueError("SWD inputs must have shape [N, C, H, W].")
    if image1.size() != image2.size():
        raise ValueError(
            f"SWD requires equal input shapes, got {tuple(image1.shape)} and "
            f"{tuple(image2.shape)}. Deterministically subsample equal counts first."
        )
    if image1.shape[1] <= 0:
        raise ValueError("SWD inputs must contain at least one channel.")
    if image1.shape[-2:] != image2.shape[-2:]:
        raise ValueError("SWD inputs must share the same spatial shape.")
    ...
    with torch.no_grad():
        ...
        for i_pyramid in range(n_pyramids + 1):
            ...
            indices = torch.randperm(n, generator=generator)[:n_descriptors]
