...
@torch.no_grad()
def _draw_imgsyn_diffusion_microbatched(
    draw_labs,
    draw_fg_imgs_e,
    diffusion_model,
    vae_model,
...
class DiffusionLossCalculator(nn.Module):
    """Compute the training loss for the ImgSyn diffusion model."""
    ...
    def forward(self, x_0, prompt_str, fg_imgs_e):
        ...
        pred_v_t = self.model(x_t, t, prompt_str, fg_imgs_e)
...
def train(
    ...
    with tqdm(data_loader, dynamic_ncols=False, colour="#ff924a", leave=False, ncols=120) as data:
        for labs, fg_imgs, _, syn_imgs, _, _ in data:
            if benchmark_recorder is not None:
                benchmark_recorder.before_step((labs, fg_imgs, syn_imgs))
            ...
            for mb_start, mb_end in iter_microbatch_slices(batch_size, microbatch_size):
                ...
                with build_train_autocast():
                    ...
                    loss, sp_loss, fq_loss = loss_calculator(syn_imgs_e, labs_mb, fg_imgs_e)
...
def benchmark_training_memory(
    *, output, warmup_steps=5, measure_steps=20, batch_size=1,
    ...
    """Measure the formal ImgSyn diffusion training path without saving."""
    ...
    recorder = TrainingMemoryRecorder(
        method_key="impgm",
        stage="imgsyn_diffusion_training",
        output=output,
        ...
        notes="The frozen shared VAE remains loaded as in formal training.",
    )
...
def valid(data_loader, Latent_model, loss_calculator, latent_scaling_factor):
    ...
    with torch.no_grad():
        for labs, fg_imgs, _, syn_imgs, _, _ in data_loader:
            batch_size = syn_imgs.shape[0]
            ...
            for mb_start, mb_end in iter_microbatch_slices(batch_size, microbatch_size):
                ...
                with build_train_autocast():
                    fg_imgs_e = encode_to_scaled_latent(Latent_model, fg_imgs_mb, latent_scaling_factor)
                    syn_imgs_e = encode_to_scaled_latent(Latent_model, syn_imgs_mb, latent_scaling_factor)
                    loss, sp_loss, fq_loss = loss_calculator(
                        syn_imgs_e,
                        labs_mb,
                        fg_imgs_e,
                    )
...
def _maybe_preload_weights(model):
    """Optionally initialize ImgSyn diffusion weights from a checkpoint."""
    if not PRELOAD_ENABLED:
        return
    if not PRELOAD_SOURCE_PATH:
        raise ValueError("ImgSyn diffusion preloading is enabled but source_path is empty.")
    if not Path(PRELOAD_SOURCE_PATH).is_file():
        raise FileNotFoundError(f"ImgSyn diffusion preload source not found: {PRELOAD_SOURCE_PATH}")
...
def main(resume_train=False):
    if TASK_NAME != "imgsyn_diffusion":
        raise RuntimeError(
            f"ImgSyn_Diffusion_Train requires imgsyn_diffusion.yaml, got {TASK_NAME!r}"
        )
    ...
    if os.path.exists(MODEL_SAVEPATH) and not resume_train:
        raise Exception('IMGSYN_DIFFUSION_MODEL exists and resume_train is False ^_^')

    ImgSyn_Diffusion_model = ImgSyn_Diffusion_UNet(
        ch=MODEL_CH,
        out_ch=MODEL_OUT_CH,
        ch_mult=MODEL_CH_MULT,
        attn_resolutions=MODEL_ATTN_RESOLUTIONS,
    ...
    optimizer = torch.optim.Adam(
        ImgSyn_Diffusion_model.parameters(),
        lr=LEARNING_RATE,
        betas=ADAM_BETAS,
    )
    ...
    if not resume_train:
        _maybe_preload_weights(ImgSyn_Diffusion_model)
    ema_model = build_ema_model(ImgSyn_Diffusion_model)
    resume_extra = {}

    if resume_train:
        ImgSyn_Diffusion_model, optimizer, lr_scheduler, last_epoch, last_loss_dict, resume_extra, _ = load_model(
            MODEL_SAVEPATH,
            ImgSyn_Diffusion_model,
            optimizer,
    ...
    draw_labs, draw_fg_imgs, _, _, _, _ = next(iter(draw_loader))
    ...
    for epoch_idx in range(start_epoch, TRAIN_MAX_EPOCHS):
        ...
        if resume_train and epoch_idx == start_epoch:
            ...
            train_loss, train_sp_loss, train_fq_loss = train(train_loader, Latent_model, optimizer, loss_calculator, latent_scaling_factor, ema_model=ema_model, training_state=training_state)
            ...
            loss_calculator.model = ImgSyn_Diffusion_model
        ...
        writer.add_scalars('ImgSyn_Diffusion/train_loss',    {f"{_tag_prefix}_train_loss": train_loss},       epoch_num)
        writer.add_scalars('ImgSyn_Diffusion/train_sp_loss', {f"{_tag_prefix}_train_sp_loss": train_sp_loss}, epoch_num)
        writer.add_scalars('ImgSyn_Diffusion/train_fq_loss', {f"{_tag_prefix}_train_fq_loss": train_fq_loss}, epoch_num)
        writer.add_scalars('ImgSyn_Diffusion/valid_loss',    {f"{_tag_prefix}_valid_loss": valid_loss},       epoch_num)
        writer.add_scalars('ImgSyn_Diffusion/valid_sp_loss', {f"{_tag_prefix}_valid_sp_loss": valid_sp_loss}, epoch_num)
        writer.add_scalars('ImgSyn_Diffusion/valid_fq_loss', {f"{_tag_prefix}_valid_fq_loss": valid_fq_loss}, epoch_num)
        ...
        if epoch_num == 1:
            rgb_save_path = os.path.join(RGB_DIR, 'Fg.png')
            tif_save_path = os.path.join(TIF_DIR, 'Fg.tif')
            save_rgb_datas(prepare_rgb_vis_tensor(draw_fg_imgs), nrow=3, savepath=rgb_save_path)
            if SAVE_TIF_IMAGES:
                save_tif_datas(
                    draw_fg_imgs,
                    projections=None,
        ...
        if epoch_num % DRAW_INTERVAL_EPOCHS == 0:
            print(f'   ImgSyn_Diffusion at epoch: {epoch_num} --- labels: {draw_labs}')
            ema_model.eval()
            Denoised_Syn_Imgs = _draw_imgsyn_diffusion_microbatched(
                draw_labs,
                draw_fg_imgs_e,
            ...
            save_rgb_datas(prepare_rgb_vis_tensor(Denoised_Syn_Imgs).cpu(), nrow=3,
                           savepath=os.path.join(RGB_DIR, f'Diffusion_RGBs_{epoch_num}.png'),
                           is_showminmax=True)

            if SAVE_TIF_IMAGES and epoch_num % TIF_INTERVAL_EPOCHS == 0:
                save_tif_datas(Denoised_Syn_Imgs, projections=None, geotransforms=None,
                               savepath=os.path.join(TIF_DIR, f'ImgSyn_Diffusion_TIF_{epoch_num}.tif'),
                               is_showminmax=True)
